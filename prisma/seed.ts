...
async function main() {
  ...
  let generalQuiz = await prisma.quiz.findFirst({
    where: { title: "General Knowledge Quiz" },
  });
  if (!generalQuiz) {
    generalQuiz = await prisma.quiz.create({
      data: {
  ...
  let scienceQuiz = await prisma.quiz.findFirst({
    where: { title: "Basic Science Quiz" },
  });
  if (!scienceQuiz) {
    scienceQuiz = await prisma.quiz.create({
      data: {
  ...
  const q2 = await prisma.question.create({
    data: {
      ...
      question_text:
        "Is water composed of two hydrogen atoms and one oxygen atom?",
      points: 1,
      display_order: 2,
    },
  });

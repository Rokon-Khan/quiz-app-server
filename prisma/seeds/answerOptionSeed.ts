...
export async function seedAnswerOptions() {
  ...
  const optionTexts = [
    ...
    'Correct Answer', 'Wrong Answer', 'Partially Correct',
  ...
  selectedQuestions.forEach((question, qIndex) => {
    ...
    for (let i = 0; i < numOptions; i++) {
      ...
      const optionText = question.question_type === 'yes_no' 
        ? (i === 0 ? 'Yes' : 'No')
      ...
      answerOptions.push({
        ...
        option_image_url: Math.random() > 0.8 ? `https://via.placeholder.com/200x150/FF6B6B/FFFFFF?text=Option+${i + 1}` : null,

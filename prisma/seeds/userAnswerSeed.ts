...
export async function seedUserAnswers() {
  ...
  selectedAttempts.forEach((attempt) => {
    ...
    questions.forEach((question) => {
      ...
      const incorrectOptions = availableOptions.filter(opt => !opt.is_correct);
      
      let selectedOptions;
      if (question.question_type === 'checkbox') {
        // For checkbox, select 1-3 options
        const numToSelect = Math.floor(Math.random() * 3) + 1;
        const optionsToChooseFrom = isCorrect ? correctOptions : availableOptions;
        ...
        const optionsToChooseFrom = isCorrect ? correctOptions : incorrectOptions;
        selectedOptions = optionsToChooseFrom.length > 0 
          ? [optionsToChooseFrom[Math.floor(Math.random() * optionsToChooseFrom.length)].id]
      ...
      userAnswers.push({
        ...
        answered_at: new Date(attempt.started_at.getTime() + Math.floor(Math.random() * 1800000)) // Random time during attempt
      });
    });
  });

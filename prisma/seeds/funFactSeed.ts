...
export async function seedFunFacts() {
  ...
  const factTitles = [
    'Did You Know?', 'Fun Fact', 'Interesting Info', 'Quick Tip', 'Amazing Fact',
    'Cool Detail', 'Bonus Info', 'Trivia Time', 'Knowledge Nugget', 'Fact Check'
  ];
  ...
  selectedQuestions.forEach((question, index) => {
    ...
    funFacts.push({
      ...
      title: `${title} #${index + 1}`,
      content: `${content} This relates to the question about "${question.question_text.substring(0, 50)}..."`,
      image_url: Math.random() > 0.6 ? `https://via.placeholder.com/300x200/4ECDC4/FFFFFF?text=Fun+Fact+${index + 1}` : null
    });
  });
